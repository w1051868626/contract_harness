...
# scripts/ 实用脚本（独立入口，部分等价于 CLI 子命令）
# RAG eval 报告三项分析（等价于 kb eval run --analyze）
...
# Windows 后台跑 A/B（DETACHED_PROCESS 标志，绕过 MSYS bash 退出杀子进程）
...
- 2026-07-25: RAG eval 报告三项分析脚本——新增 `scripts/analyze_rag_eval.py`，基于已产出的 `summary.csv`/`details.csv` 做三项分析：①`hit1_miss_hit3_hit.csv`（hit@1=0 但 hit@3=1 的 query，排序改进空间）②`chunk_hit_distribution.csv`/`chunk_systematic_miss.csv`（按 expected chunk 维度的命中率 + 系统性漏检 hit1_rate≤0.3 且 query≥3）③`rag_eval_charts.html`（pyecharts 交互式图表，缩放/悬停 tooltip/dataZoom）+ `rag_eval_analysis_summary.md`（Markdown 摘要）；同步更新 README.md/AGENTS.md/CLAUDE.md；新增 `pyecharts` 依赖到 contract-harness 环境。
...
- 2026-07-26: 新代码全量 eval 对比报告——用候选池扩大 + merge 后 rerank 的新代码跑全量 20,442 query eval，与 07-25 baseline 对比：hit_rate@1 0.5239→0.5316（+0.77pp）、hit_rate@5 0.9021→0.9396（+3.75pp）、top-5 仍漏 2,001→1,234（-38%）；候选池扩大主要解决召回（top-5），top-1 瓶颈仍待 Reranker 调优；系统性漏检 chunk 从 477 增到 504（候选池扩大后更多 chunk 进入评估）；详见 `.harness/reports/eval_compare_2026-07-26.md`。
- 2026-07-26: Reranker top_n 扩大给 top-1 判别更大参考面——`OpenAIReranker.rerank` 的 `/rerank` API `top_n` 从 `top_k` 改为 `len(candidates)`（让 API 对全 pool �候选打分），客户端按 `relevance_score` 降序后截 top_k；原 top_n=top_k 会截断打分阶段，正确 chunk 在 pool 内但分未排进 top_k 时直接丢失；62 个 test_rag 用例全过，ruff/pyright 全清。
- 2026-07-26: Reranker 输入注入法条标题前缀——`OpenAIReranker.rerank` 喂给 `/rerank` API 的 `documents` 从裸 `chunk.content` 改为 `【{law_name}·{articles}】{chunk.content}`（注入法条标题前缀），**不动 `chunk.content`**（其他调用方不受影响）；新增 `_format_for_rerank(chunk)` 静态方法，无 metadata 或缺 `articles`/`law_name` 时降级为裸 content；200 样本 A/B 对照（pool 均为 20，仅差是否注入前缀）top-1 hit_rate 提升 +9.5pp（0.265 → 0.360），效果显著；详见 `.harness/reports/eval_compare_2026-07-26.md` Section 7.2。
